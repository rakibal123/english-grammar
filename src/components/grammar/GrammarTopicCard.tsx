    ...
    <div
      className={`card group transition-all duration-200 ${
        isLocked
          ...
          : 'hover:shadow-md hover:border-[#4F46E5]/30 cursor-pointer'
      }`}
    >
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center justify-between gap-2 mb-1">
            <h3 className="font-semibold text-[#0B1C30] text-sm">{topic.title}</h3>
            <div className="flex items-center gap-2 flex-shrink-0">
              {isCompleted && <CheckCircle2 className="w-4 h-4 text-green-500" />}
              {!isLocked && <MasteryBadge level={masteryLevel as 'beginner' | 'learning' | 'improving' | 'mastered'} />}
            </div>
          </div>
          <p className="text-xs text-[#76777D] mb-3 line-clamp-1">{topic.description}</p>
          {!isLocked && (
            <div>
              <div className="flex justify-between items-center mb-1">
                <span className="text-[11px] text-[#76777D]">Progress</span>
                <span className="text-[11px] font-medium text-[#4F46E5]">{completion}%</span>
              </div>
