...
export default function LearnClient() {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-6 lg:py-8">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-foreground mb-2">Grammar Journey</h1>
      ...
      <div className="space-y-3">
        {topicsLoading
          ...
          : topics.map((topic, i) => {
              ...
              const prevProgress = prevTopic ? progressMap[prevTopic._id] : null;
              const isPrevCompleted = i === 0 || (prevProgress?.completion ?? 0) >= 100;
              const isEffectivelyLocked = !isPrevCompleted;
                ...
                return (
                  <div key={topic._id} className="card opacity-60">
                    <div className="flex items-center gap-4">
                      ...
                      <span className="text-xs text-lighter bg-slate-100 dark:bg-slate-800 px-2 py-0.5 rounded-full">
                        Locked
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-1.5">
                        ...
                        <span className="text-xs text-lighter flex-shrink-0">{completion}%</span>
                      </div>
                    </div>
                    <ChevronRight className="w-4 h-4 text-lighter group-hover:text-primary transition-colors flex-shrink-0" />
                  </div>
                </Link>
              );
            })}
