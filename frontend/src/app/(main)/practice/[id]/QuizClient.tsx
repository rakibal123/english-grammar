...
import { validateAnswer } from '@/lib/utils/answerValidation';
...
export default function QuizClient({ testSetId }: { testSetId: string }) {
  ...
  const [result, setResult] = useState<{ percentage: number; correctCount: number; incorrectCount: number; skippedCount: number; xpEarned: number; attemptId: string } | null>(null);
  ...
  const handleReveal = useCallback(() => {
    ...
    const isCorrect = validateAnswer(answer, accepted);
  ...
  if (phase === 'result' && result) {
    ...
    const scoreColor = score >= 80 ? 'var(--correct-border)' : score >= 60 ? 'var(--badge-improving-text)' : 'var(--incorrect-border)';
    const scoreBg = score >= 80 ? 'var(--correct-bg)' : score >= 60 ? 'var(--badge-improving-bg)' : 'var(--incorrect-bg)';
    ...
    return (
      <div className="max-w-xl mx-auto px-4 sm:px-6 py-8">
        <div className="text-center mb-8">
          ...
          <p className="text-lighter">{testSet?.title}</p>
        </div>

        <div className="card mb-6 text-center">
          <p className="text-6xl font-bold mb-2" style={{ color: scoreColor }}>{score}%</p>
          <div className="flex h-3 w-full rounded-full overflow-hidden mb-4 bg-gray-200 dark:bg-gray-700">
            <div style={{ width: `${(result.correctCount / questions.length) * 100}%` }} className="bg-green-500" />
            <div style={{ width: `${(result.incorrectCount / questions.length) * 100}%` }} className="bg-red-500" />
            <div style={{ width: `${((result.skippedCount || 0) / questions.length) * 100}%` }} className="bg-gray-400" />
          </div>
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 pt-4 border-t border-[var(--border-color)]">
            <div>
              ...
              <p className="text-xs text-lighter">Correct</p>
            </div>
            <div>
              <p className="text-2xl font-bold text-red-500">{result.incorrectCount}</p>
              <p className="text-xs text-lighter">Wrong</p>
            </div>
            <div>
              <p className="text-2xl font-bold text-gray-500">{result.skippedCount || 0}</p>
              <p className="text-xs text-lighter">Skipped</p>
            </div>
            <div>
              <p className="text-2xl font-bold text-primary flex items-center justify-center gap-1">
                <Zap className="w-5 h-5" />{result.xpEarned}
              </p>
              <p className="text-xs text-lighter">XP Earned</p>
            </div>
          </div>
        </div>
        ...
        {result.incorrectCount > 0 && (
          <div className="card mb-4 border-red-100 dark:border-red-900/30">
            ...
            <div className="space-y-4">
              {Object.values(answers).filter(a => !a.isCorrect).map((a, i) => {
                ...
                if (!q) return null;
                return (
                  <div key={i} className="bg-red-50 dark:bg-red-900/10 border border-red-100 dark:border-red-900/20 rounded-lg p-3">
                    <p className="text-sm font-medium text-foreground mb-2">{q.questionText}</p>
                    <p className="text-xs text-red-600 dark:text-red-400">Your answer: <strong>{a.userAnswer || '(Skipped)'}</strong></p>
                    <p className="text-xs text-green-600 dark:text-green-400">Correct: <strong>{a.correctAnswer}</strong></p>
                    <p className="text-xs text-lighter mt-1.5 italic">{a.explanation}</p>
                  </div>
                );
              })}
            </div>
  ...
  if (phase === 'review') {
    ...
    return (
      <div className="max-w-xl mx-auto px-4 sm:px-6 py-8">
        <h2 className="text-xl font-bold text-foreground mb-2">Review Answers</h2>
            ...
            return (
              <button
                ...
                className={`w-9 h-9 rounded-lg text-xs font-bold border-2 transition-colors ${
                  !a ? 'border-[var(--border-color)] text-lighter' :
  ...
  return (
    <div className="max-w-2xl mx-auto px-4 sm:px-6 py-6">
      ...
      <div className="flex items-center gap-4 mb-4">
        <Link href="/learn" className="text-lighter hover:text-foreground transition-colors">
          <ArrowLeft className="w-5 h-5" />
        </Link>
        <div className="flex-1">
          <div className="flex items-center justify-between text-xs text-lighter mb-1">
            <span>{testSet?.title}</span>
            <span>{currentIdx + 1}/{questions.length}</span>
          </div>
          <ProgressBar value={progress} height="h-1.5" />
        </div>
        <div className="flex items-center gap-1 text-lighter text-xs">
      ...
      <div className="card mb-4">
        <div className="flex items-center gap-2 mb-4">
          ...
          <span className="text-[10px] text-lighter uppercase tracking-wider">{currentQuestion.type.replace('_', ' ')}</span>
        </div>

        <h2 className="text-foreground font-semibold text-lg leading-relaxed mb-1">
          {currentQuestion.questionText}
        </h2>
        {currentQuestion.banglaText && (
          <p className="text-sm text-lighter mb-4">{currentQuestion.banglaText}</p>
        )}
            ...
            {currentQuestion.options.map((opt: string, i: number) => {
              ...
              return (
                <button
                  ...
                  className={optClass}
                >
                  <span className="w-6 h-6 rounded-md bg-[var(--bg-color)] border border-[var(--border-color)] text-xs font-bold flex items-center justify-center flex-shrink-0">
                    {label}
                  </span>
                  {opt}
          ...
          <div className="flex items-start gap-2">
            ...
            <div>
              ...
              {currentQuestion.banglaExplanation && (
                <p className="text-sm text-lighter mt-1">{currentQuestion.banglaExplanation}</p>
              )}
            </div>
          </div>
        </div>
