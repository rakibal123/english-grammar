  ...
  return (
    <div className="max-w-5xl mx-auto px-4 sm:px-6 py-6 lg:py-8">
      ...
      <div className="flex items-start justify-between mb-8">
        <div>
          <p className="text-sm text-lighter mb-1">{getGreeting()}</p>
          <h1 className="text-2xl font-bold text-foreground">
            {user?.username || 'Learner'}
            {user?.isGuest && (
              <span className="ml-2 text-xs font-normal text-lighter bg-[#F1F5F9] px-2 py-0.5 rounded-full">
                Guest
              </span>
            )}
          </h1>
        ...
        <div className="flex items-center gap-3">
          <Link href="/profile" className="flex items-center gap-1.5 bg-[var(--streak-bg)] text-[var(--streak-text)] px-3 py-1.5 rounded-xl hover:opacity-80 transition-opacity">
            <Flame className="w-4 h-4" />
            <span className="text-sm font-bold">{user?.currentStreak ?? 0}</span>
          </Link>
          <Link href="/progress" className="flex items-center gap-1.5 bg-[var(--nav-active-bg)] text-primary px-3 py-1.5 rounded-xl hover:opacity-80 transition-opacity">
      ...
      <div className="grid grid-cols-3 gap-4 mb-8">
        <div className="card py-4">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs text-lighter font-medium">Overall Progress</span>
          </div>
        ...
        <div className="card py-4">
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs text-lighter font-medium">Mastered</span>
          </div>
          <p className="text-2xl font-bold text-foreground">{masteredCount}/12</p>
          <p className="text-xs text-lighter mt-1">Topics</p>
        </div>
        <div className="card py-4">
          <div className="flex items-center gap-2 mb-1">
            <Target className="w-4 h-4 text-green-500" />
            <span className="text-xs text-lighter font-medium">Tests Done</span>
          </div>
          <p className="text-2xl font-bold text-foreground">
            {progress.reduce((sum, p) => sum + p.testsCompleted, 0)}
          </p>
          <p className="text-xs text-lighter mt-1">Practice sets</p>
        </div>
      </div>

      {/* Continue Learning */}
      <div className="mb-8">
        <h2 className="text-base font-semibold text-foreground mb-4">Continue Learning</h2>
        <Link
        ...
        >
          <div className="flex items-center gap-4">
            ...
            <div className="flex-1">
              <p className="text-xs text-lighter mb-0.5">Next Up</p>
              ...
              <div className="flex items-center gap-3 mt-1.5">
                ...
                <span className="text-xs text-lighter flex-shrink-0">
      ...
      <div className="mb-8">
        <div className="flex items-center justify-between mb-4">
          ...
          <Link href="/practice" className="text-xs text-primary font-medium hover:underline">
            View all
          </Link>
        </div>
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {[
            ...
            <Link
              ...
              className={`card py-4 group transition-all duration-200 ${locked ? 'opacity-60 cursor-not-allowed' : 'hover:shadow-md hover:border-primary/30'}`}
            >
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-lighter mt-0.5">{meta}</p>
                </div>
              </div>
            </Link>
          ))}
      ...
      {recentAttempts.length > 0 && (
        <div>
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-base font-semibold text-foreground">Recent Tests</h2>
          ...
          <div className="space-y-2">
            {recentAttempts.slice(0, 5).map((attempt) => (
              <div key={attempt._id} className="card py-3 px-4 flex items-center gap-4">
                ...
                <div className="flex-1">
                  <p className="text-sm font-medium text-foreground">{attempt.topicId?.title}</p>
                  <p className="text-xs text-lighter">{attempt.testSetId?.title}</p>
                </div>
                <MasteryBadge level={
      ...
      {user?.isGuest && (
        <div className="mt-8 bg-gradient-to-br from-[#4F46E5] to-[#7C3AED] rounded-2xl p-6 text-white">
          ...
          <Link
            href="/register"
            className="inline-flex items-center gap-2 bg-[var(--card-bg)] text-primary font-semibold text-sm px-5 py-2.5 rounded-lg hover:bg-indigo-50 transition-colors"
          >
            Create Account
