...
import { updateStreak } from '@/actions/user';
  ...
  try {
    ...
    const validatedAnswers = await Promise.all(
      answers.map(async (a: { questionId: string; userAnswer: string }) => {
        ...
        const isCorrect = validateAnswer(a.userAnswer, accepted);
    ...
    const skippedCount = valid.filter(a => !a!.isCorrect && !a!.userAnswer.trim()).length;
    const actualIncorrectCount = valid.length - correctCount - skippedCount;
    const totalQuestions = valid.length || 1; // Prevent division by zero
    const percentage = valid.length > 0 ? Math.round((correctCount / valid.length) * 100) : 0;
    ...
    await UserProgress.findOneAndUpdate(
      { userId, topicId },
      { attempts: newAttempts, testsCompleted: newAttempts, averageScore: newAvg, bestScore: newBest, mastery, masteryLevel, completion, lastActivityAt: new Date(), $inc: { mistakeCount: actualIncorrectCount } },
      { upsert: true }
    );
    ...
    return NextResponse.json({ 
      ...
      percentage, 
      correctCount, 
      incorrectCount: actualIncorrectCount,
      skippedCount,
      xpEarned 
    });
  } catch (error) {
